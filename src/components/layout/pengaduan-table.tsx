...
interface Props {
  ...
  klasifikasiList?: { id: string; nama: string }[];
  statusFilter?: string;
  unitFilter?: string;
...
export function PengaduanTable({
  ...
  // State for Modals
  const [activeBerkas, setActiveBerkas] = useState<
    { id: string; nomor_berkas: string; judul: string }[]
  >([]);
  ...
  const [tahapSaatDamai, setTahapSaatDamai] = useState<
    'Perdamaian Sebelum Penyelidikan' | 'Perdamaian Setelah Penyelidikan'
  >('Perdamaian Sebelum Penyelidikan');

  useEffect(() => {
    if (isPerdamaianOpen && selectedRow) {
      const isSetelahLidik =
        selectedRow.berkas ||
        !['diterima', 'registrasi', 'verifikasi', 'disposisi'].includes(selectedRow.status);
      // eslint-disable-next-line react-hooks/set-state-in-effect
      setTahapSaatDamai(
        isSetelahLidik ? 'Perdamaian Setelah Penyelidikan' : 'Perdamaian Sebelum Penyelidikan',
      );
    }
  ...
  const handleSyaratClick = (
    key: keyof typeof syaratPerdamaian,
  ...
  const handleDocClick = (
    key: keyof typeof tindakLanjutDocs,
    e: React.MouseEvent<HTMLElement>,
  ): void => {
    if (e.ctrlKey) {
      e.preventDefault();
  ...
  const columns = useMemo<ColumnDef<PengaduanRow>[]>(
    () => [
      ...
      },
      {
        ...
        header: 'Jenis',
        cell: ({ row }) => (
          <span className="font-medium whitespace-normal max-w-[100px] block leading-tight">
            {row.original.jenis}
          </span>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <div className="text-center whitespace-nowrap w-[80px]">
            {row.original.tgl_pengaduan || '-'}
          </div>
        ),
      },
      {
        ...
        header: 'Pelapor',
        cell: ({ row }) => (
          <span className="whitespace-normal block max-w-[120px]">
            {formatNameCase(row.original.pelapor_nama) || 'Anonim'}
          </span>
        ),
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const hasLengkap = !!(
            ...
            row.original.kronologi_lengkap !== row.original.kronologi
          );
          const displayText =
          ...
          return (
            <div className="flex flex-col gap-1.5 w-full min-w-[250px]">
              <div className="whitespace-pre-wrap text-justify">{displayText}</div>
              {kronologiLocked && hasLengkap && (
                <span className="text-[10px] text-primary/60 font-medium">
                  Ctrl+Shift: kronologi asli (terkunci)
                </span>
              )}
            </div>
          );
      ...
      },
      {
        ...
        header: 'Unit',
        cell: ({ row }) => (
          <span className="whitespace-nowrap w-[60px] block">
            {(row.original.unit as { nama: string })?.nama || '-'}
          </span>
        ),
      },
      {
  ...
  const isSyaratLengkap =
    syaratPerdamaian.materil_keresahan &&
  ...
  return (
    <div className="space-y-4 flex flex-col flex-1 min-h-0">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select value={unitFilter || 'all'} onValueChange={(v) => updateFilter('unit', v ?? '')}>
          <SelectTrigger className="h-8 w-[180px] text-xs">
            <SelectValue placeholder="Unit/UR" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">Semua Unit</SelectItem>
            <SelectItem value="undistributed">Subbid (belum distribusi)</SelectItem>
            {units.map((u) => (
        ...
        <Select
          value={klasifikasiFilter || 'all'}
          onValueChange={(v) => updateFilter('klasifikasi', v ?? '')}
        >
        ...
        <Select
          value={atensiFilter || 'all'}
          onValueChange={(v) => updateFilter('atensi', v ?? '')}
        >
          <SelectTrigger className="h-8 w-[120px] text-xs">
            <SelectValue placeholder="Atensi" />
          </SelectTrigger>
          <SelectContent>
      ...
      <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Tindakan ini akan membatalkan pengaduan aktif. Silakan masukkan alasan pembatalan
              untuk arsip log audit.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-2">
              ...
              {activeBerkas.length === 0 ? (
                <p className="text-xs text-muted-foreground italic">
                  Tidak ada berkas aktif yang tersedia.
                </p>
              ) : (
                <Select
                  value={selectedBerkasId}
                  onValueChange={(val) => setSelectedBerkasId(val || '')}
                >
      ...
      <Dialog open={isSplitOpen} onOpenChange={setIsSplitOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Apakah Anda yakin ingin memisahkan pengaduan ini dari berkas yang terhubung? Tindakan
              ini akan mengembalikan status pengaduan menjadi bebas.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleSplit} disabled={isSubmitting}>
            ...
            <div className="space-y-3.5 py-1">
              ...
              <div className="space-y-1">
                ...
                <Select
                  ...
                  onValueChange={(val) =>
                    setTahapSaatDamai(
                      val as 'Perdamaian Sebelum Penyelidikan' | 'Perdamaian Setelah Penyelidikan',
                    )
                  }
                >
                  ...
                  <label className="flex items-start space-x-1.5 cursor-pointer select-none">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        setSyaratPerdamaian((prev) => ({
                          ...prev,
                          materil_keresahan: checked as boolean,
                        }))
                      }
                  ...
                  <label className="flex items-start space-x-1.5 cursor-pointer select-none">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        setSyaratPerdamaian((prev) => ({
                          ...prev,
                          materil_konflik: checked as boolean,
                        }))
                      }
                  ...
                  <label className="flex items-start space-x-1.5 cursor-pointer select-none">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        setSyaratPerdamaian((prev) => ({
                          ...prev,
                          materil_tidak_keberatan: checked as boolean,
                        }))
                      }
                      ...
                      <label className="flex items-start space-x-1.5 cursor-pointer select-none">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            setSyaratPerdamaian((prev) => ({
                              ...prev,
                              materil_mensrea: checked as boolean,
                            }))
                          }
                          onClick={(e) => handleSyaratClick('materil_mensrea', e)}
                        />
                        <span className="font-medium leading-tight text-black text-sm">
                          a. tingkat kesalahan pelaku tidak berat dengan mempertimbangkan niat and
                          tujuan pelaku (Mensrea);
                        </span>
                      </label>
                      <label className="flex items-start space-x-1.5 cursor-pointer select-none">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            setSyaratPerdamaian((prev) => ({
                              ...prev,
                              materil_bukan_residivis: checked as boolean,
                            }))
                          }
                          onClick={(e) => handleSyaratClick('materil_bukan_residivis', e)}
                        />
                        <span className="font-medium leading-tight text-black text-sm">
                          b. pelaku bukan anggota yang sering melakukan pelanggaran Disiplin
                          dan/atau KEPP dan atas pertimbangan ankum layak untuk dilakukan
                          perdamaian.
                        </span>
                      </label>
                    </div>
                  ...
                  <label className="flex items-start space-x-1.5 cursor-pointer select-none">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        setSyaratPerdamaian((prev) => ({
                          ...prev,
                          formil_permohonan: checked as boolean,
                        }))
                      }
                  ...
                  <label className="flex items-start space-x-1.5 cursor-pointer select-none">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        setSyaratPerdamaian((prev) => ({
                          ...prev,
                          formil_pernyataan: checked as boolean,
                        }))
                      }
                  ...
                  <label className="flex items-start space-x-1.5 cursor-pointer select-none">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        setSyaratPerdamaian((prev) => ({
                          ...prev,
                          formil_pencabutan: checked as boolean,
                        }))
                      }
                  ...
                  <label className="flex items-start space-x-1.5 cursor-pointer select-none">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        setSyaratPerdamaian((prev) => ({
                          ...prev,
                          formil_bap_tambahan: checked as boolean,
                        }))
                      }
                    ...
                    <label className="flex items-start space-x-1.5 cursor-pointer select-none">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          setSyaratPerdamaian((prev) => ({
                            ...prev,
                            sebelum_pencabutan_diteliti: checked as boolean,
                          }))
                        }
                    ...
                    <label className="flex items-start space-x-1.5 cursor-pointer select-none">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          setSyaratPerdamaian((prev) => ({
                            ...prev,
                            sebelum_klarifikasi_pencabutan: checked as boolean,
                          }))
                        }
                    ...
                    <div className="flex items-center justify-between py-1 border-b border-slate-100 last:border-0">
                      <label className="flex items-start space-x-1.5 cursor-pointer select-none">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            setTindakLanjutDocs((prev) => ({
                              ...prev,
                              ba_pemeriksaan_tambahan: checked as boolean,
                            }))
                          }
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          handleGenerateSingleDoc(
                            'BA-BAP-TAMBAHAN',
                            'Berita Acara Pemeriksaan Tambahan 2 pihak',
                            'ba_pemeriksaan_tambahan',
                          )
                        }
                      ...
                      <label className="flex items-start space-x-1.5 cursor-pointer select-none">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            setTindakLanjutDocs((prev) => ({
                              ...prev,
                              nd_saran_penghentian: checked as boolean,
                            }))
                          }
                      ...
                      <label className="flex items-start space-x-1.5 cursor-pointer select-none">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            setTindakLanjutDocs((prev) => ({
                              ...prev,
                              sprin_henti: checked as boolean,
                            }))
                          }
