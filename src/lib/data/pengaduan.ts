  ...
  const [vectorRes, kronoRes, terlaporLinkRes] = await Promise.all([
    ...
    (async () => {
      const { data: terlaporRows } = await supabase
        ...
        .eq('tenant_id', tenantId)
        .or(
          `nama.ilike.${likePattern},pangkat.ilike.${likePattern},nrp.ilike.${likePattern},jabatan.ilike.${likePattern},kesatuan.ilike.${likePattern}`,
        );
      if (!terlaporRows || terlaporRows.length === 0)
        return { data: [] as { pengaduan_id: string }[] };
      return supabase
...
export async function getPengaduanList(filters: SearchFilters = {}) {
  ...
  let query = supabase
    .from('pengaduan')
    .select(
      '*, klasifikasi(nama), unit:organizations(nama), pengaduan_terlapor(terlapor(nama, pangkat, nrp, jabatan, kesatuan)), berkas:berkas(id, nomor_berkas)',
      { count: 'exact' },
    )
    .eq('tenant_id', tenantId)
  ...
  if (filters.unitId === 'undistributed') {
...
export async function createPengaduan(data: Partial<Pengaduan> & { terlapor_nama?: string }) {
  ...
  const isLI =
    data.jenis?.toUpperCase() === 'LAPORAN_INFORMASI' ||
    data.jenis?.toUpperCase() === 'LAPORAN INFORMASI';
...
export async function updatePengaduan(
  id: string,
  data: Partial<Pengaduan & { terlapor_nama?: string }>,
) {
  const supabase = await createClient();
  const tenantId = await requireTenant();
...
export async function splitPengaduanFromBerkas(pengaduanId: string) {
  ...
  const nomorBerkas =
    (prev.berkas as unknown as Array<{ nomor_berkas: string }>)?.[0]?.nomor_berkas || 'unknown';
  await supabase.from('audit_logs').insert({
    tenant_id: tenantId,
    action: 'UPDATE',
    entity_type: 'pengaduan',
...
export async function getAnevJenis(unitId?: string) {
  ...
  let query = supabase
    .from('pengaduan')
    .select('klasifikasi_id, status, klasifikasi!inner(nama)')
    .eq('tenant_id', tenantId);
  if (unitId) query = query.eq('unit_id', unitId);
  ...
  for (const row of data) {
    ...
    if (!grouped[k])
      grouped[k] = { nama: k, jumlah: 0, proses: 0, selesai: 0, terbukti: 0, tidak_terbukti: 0 };
    grouped[k].jumlah++;
    if (['registrasi', 'verifikasi', 'disposisi', 'lidik_berjalan', 'gelar'].includes(row.status))
      grouped[k].proses++;
...
export async function getAnevUnit(): Promise<
  Array<{
    unit: string;
    unit_id: string;
    jumlah: number;
