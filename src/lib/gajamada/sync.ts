...
function getFetchParams(vars: Record<string, string>): GajamadaFetchParams {
  ...
  try { statusExclude = JSON.parse(statusExcludeRaw) } catch { statusExclude = [] }
  ...
  return {
    ...
    disposisiPolda: vars['nama_polda'] || '',
    disposisiCasePosition: vars['disposisi_kasubbid'] || '',
    statusExclude,
    size: 10,
  }
}
...
export async function syncGajamadaIntake({
  ...
  try {
    ...
    const existingSet = new Set((existingIds || []).map((r) => r.gajamada_id).filter(Boolean) as string[])
    ...
    const pengaduanRows = reports.map((r) => {
      const katKode = normalizeCategoryName(r.category).toUpperCase().replace(/[^A-Z0-9]/g, '_')
      return {
        tenant_id: tenantId,
        gajamada_id: r.id,
